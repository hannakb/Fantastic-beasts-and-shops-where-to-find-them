    ...
    function(e) {
      ...
      Diagnostics.log(macAddress);
		  ...
		  Networking.fetch(url, request).then(function(result) {
		      ...
		      // Diagnostics.log("fdsfsdfgsdfhsdf")
		      // Diagnostics.log(Scene.root.find('rectangle1').hidden);
		      // Scene.root.find('rectangle1').hidden = true;
		      Scene.root.find('externalMacRectangle').hidden = true;
		      Scene.root.find('externalInfoRectangle').hidden = false;
		      // recommendationNode.hidden = false;
			  recommendationNode.text = "Go to ("+json[0][0][0]+","+json[0][0][1]+") to find a new world!";
